...
function renderLearningList(){
  ...
  SKILL_LEARNING_REGISTRY.forEach(skill=>{
    const state=skillLearningState(skill.id),cap=getLearningDescriptor(skill.id).capabilities,row=document.createElement('div');
    row.className=`learning-skill-row ${state.trained?'trained':''} ${!cap.trainable?'disabled-learning':''}`;
