...
export class SkillExecutor{
  ...
  capabilityCheck(skillId){return this.environment?environmentSupportsSkill(this.environment,skillId):{ok:true,missing:[],skillId}}
  async execute(step){
    const spec=skillSpecs[step.name];if(!spec)return{ok:false,reason:`unknown_skill:${step.name}`};
    const capability=this.capabilityCheck(step.name);if(!capability.ok)return{ok:false,reason:`environment_capability_missing:${capability.missing.join(',')}`,environmentId:capability.environmentId||this.environment?.id||null,missingCapabilities:capability.missing};
    const args=step.args||{},state=this.environment?.getState?.()||this.store.state,pre=spec.pre(state,args);if(pre!=='ok')return{ok:false,reason:`precondition_failed:${pre}`};
