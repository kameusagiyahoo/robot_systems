...
export class RulePolicyBase extends PolicyInterface{
  ...
  async execute(skill,args={}){
    const s=this.state(),sim=this.controlConfig();
    switch(skill){
      case'navigate_to_pallet':{const p=s.pallets[args.palletId];if(!p)return{ok:false,reason:'pallet_not_found'};const m=await this.followPath(this.palletApproachPath(p),{tolerance:12,maxTicks:1100});return m.ok?{ok:true,message:`approached ${p.label} via pre-align path (${sim.controller})`,ticks:m.ticks,meanCrossTrackError:m.meanCrossTrackError??null,policy:m.policy}:{ok:false,reason:m.reason}}
      case'detect_pallet':{const p=s.pallets[args.palletId];if(!p)return{ok:false,reason:'pallet_not_found'};if(s.failures?.forceDetectionFailure)return{ok:false,reason:'forced_detection_failure'};const d=this.worldDistance(s.robot,p),visible=this.hasService('perception.palletVisible')?this.service('perception.palletVisible',p,s.robot):d<180;if(!visible)return{ok:false,reason:'pallet_not_visible'};if(this.hasService('perception.markDetected'))this.service('perception.markDetected',args.palletId);else if(!s.perception.detectedPallets.includes(args.palletId)){s.perception.detectedPallets.push(args.palletId);this.emit()}return{ok:true,message:`${p.label} detected (${d.toFixed(1)} ${this.lengthUnit()})`}}
      case'align_to_pallet':{const p=s.pallets[args.palletId];if(!p)return{ok:false,reason:'pallet_not_found'};if(s.failures?.forceAlignmentFailure)return{ok:false,reason:'forced_alignment_failure'};const m=await this.dockToPallet(p);if(!m.ok)return{ok:false,reason:m.reason,yawError:m.yawError??null};if(this.hasService('robot.setAligned'))this.service('robot.setAligned',true);else{s.robot.aligned=true;this.emit()}return{ok:true,message:`aligned by ${m.policy} (yaw error ${m.yawError.toFixed(1)}°)`,ticks:m.ticks,policy:m.policy}}
      case'insert_forks':{if(s.failures?.forceInsertionFailure)return{ok:false,reason:'forced_insertion_failure'};if(this.hasService('manipulation.insertForks'))return await Promise.resolve(this.service('manipulation.insertForks',args.palletId));s.robot.carrying=args.palletId;s.pallets[args.palletId].status='on_forks';this.emit();return{ok:true,message:'forks inserted'}}
      case'lift':{if(this.hasService('manipulation.setFork'))return await Promise.resolve(this.service('manipulation.setFork',true));const res=await this.act({type:'fork',raised:true});return res?.ok===false?res:{ok:true,message:'pallet lifted'}}
      case'navigate_to':{const l=s.locations[args.locationId];if(!l)return{ok:false,reason:'location_not_found'};const target=this.locationApproachTarget(l),m=await this.followPath(this.pathTo(target),{tolerance:12,maxTicks:1100});return m.ok?{ok:true,message:`followed path to ${l.label} (${sim.controller})`,ticks:m.ticks,meanCrossTrackError:m.meanCrossTrackError??null,policy:m.policy}:{ok:false,reason:m.reason}}
      case'place':{const l=s.locations[args.locationId],id=s.robot.carrying;if(!id)return{ok:false,reason:'no_load'};if(this.hasService('manipulation.place'))return await Promise.resolve(this.service('manipulation.place',id,args.locationId));s.pallets[id].x=l.x;s.pallets[id].y=l.y;s.pallets[id].status='placed';s.robot.carrying=null;s.robot.forkRaised=false;s.robot.aligned=false;this.emit();return{ok:true,message:`placed at ${l.label}`}}
      case'retreat':{const target=this.retreatTarget(s.robot,70),m=await this.driveTo(target,{tolerance:10,maxTicks:320,allowReverse:true,maxSpeedOverride:30});if(!m.ok)return{ok:false,reason:m.reason};if(this.hasService('agent.markRetreated'))this.service('agent.markRetreated',true);else{s.agent.memory.retreated=true;this.emit()}return{ok:true,message:'retreated in reverse',ticks:m.ticks,policy:'classic'}}
      case'avoid_obstacle':if(this.hasService('agent.setAlternateRoute'))this.service('agent.setAlternateRoute',true);else{s.agent.memory.alternateRoute=true;this.emit()}return{ok:true,message:'alternate waypoint route enabled'};
      case'reposition_for_detection':{const yaw=deg2rad(s.robot.yaw),target={x:s.robot.x-Math.sin(yaw)*45,y:s.robot.y+Math.cos(yaw)*45},m=await this.driveTo(target,{tolerance:10,maxTicks:260,allowReverse:true,maxSpeedOverride:28});return m.ok?{ok:true,message:'repositioned for another detection attempt'}:{ok:false,reason:m.reason}}
