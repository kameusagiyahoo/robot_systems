...
registerLearningVisualizationRenderer('loss_curve',(spec,context)=>{
  const el=card(spec.title),history=context.model?.lossHistory||[];
  if(history.length<2){const p=document.createElement('p');p.className='plugin-viz-note';p.textContent='再学習するとEpochごとのLoss曲線を表示します。';el.appendChild(p);return el}
  const [c,ctx]=canvas2d(el),w=c.width,h=c.height,pad=18,losses=history.map(x=>Number(x.loss)).filter(Number.isFinite),max=Math.max(...losses),min=Math.min(...losses),span=Math.max(1e-9,max-min);
  clear(ctx,w,h);ctx.strokeStyle='#17202a';ctx.lineWidth=2;ctx.beginPath();history.forEach((p,i)=>{const x=pad+(w-pad*2)*(i/Math.max(1,history.length-1)),y=h-pad-(h-pad*2)*((Number(p.loss)-min)/span);if(i===0)ctx.moveTo(x,y);else ctx.lineTo(x,y)});ctx.stroke();
  const meta=document.createElement('div');meta.className='plugin-viz-meta';meta.textContent=`epoch ${history.at(-1)?.epoch||'-'} / loss ${num(history.at(-1)?.loss,4)}`;el.appendChild(meta);return el;
});

registerLearningVisualizationRenderer('dataset_distribution',(spec,context)=>{
  const el=card(spec.title),summary=context.dataset?.featureSummary||{},preview=context.dataset?.preview||[];
  if(!Object.keys(summary).length){const p=document.createElement('p');p.className='plugin-viz-note';p.textContent='再学習するとDataset分布を保存して表示します。';el.appendChild(p);return el}
...
export function renderLearningVisualizations(host,specs=[],context={}){
  ...
  for(const spec of specs){const renderer=renderers.get(spec.type);if(!renderer)continue;host.appendChild(renderer(spec,context))}
  if(!host.children.length){const p=document.createElement('p');p.className='plugin-viz-note';p.textContent='このSkill Pluginには現在可視化が定義されていません。';host.appendChild(p)}
}
