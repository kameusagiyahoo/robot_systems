...
export function latestEvaluationForPolicy(id,policy,controller=null,modelId=null){
  const match=e=>e?.policy===policy&&(!controller||e.controller===controller)&&(!modelId||e.modelId===modelId),history=loadSkillEvaluationHistory(id);for(let i=history.length-1;i>=0;i--)if(match(history[i]))return history[i];const latest=loadSkillEvaluation(id);return match(latest)?latest:null;
}
