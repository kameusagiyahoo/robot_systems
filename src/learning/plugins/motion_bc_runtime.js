...
export class MotionBehaviorCloningRuntimeAdapter extends SkillRuntimePolicyAdapter{
  constructor(ioAdapter=motionSkillIOAdapter){super({id:'motion_bc_runtime',label:'Motion BC Runtime',version:4});this.ioAdapter=ioAdapter}
  ...
  getRequiredDomainServices(skillId){const common=['state.get','action.send','control.config','motion.ioProfile'];if(skillId==='navigate_to_pallet')return[...common,'path.palletApproach'];if(skillId==='navigate_to')return[...common,'path.to','target.locationApproach'];if(skillId==='align_to_pallet')return[...common,'target.palletDock'];if(skillId==='retreat')return[...common,'target.retreat'];return common}
  describe(skillId){return{...super.describe(skillId),requiredDomainServices:this.getRequiredDomainServices(skillId),skillIOAdapter:this.ioAdapter.describe(skillId)}}
  ...
  async learnedAlign(args,context){
    ...
    for(let i=0;i<520;i++){
      ...
      if(d<11&&yawError<24){await this.act(context,{type:'stop'});if(context.domainServices?.has?.('robot.setAligned'))context.domainServices.call('robot.setAligned',true);else{s.robot.aligned=true;this.emit(context)}return{ok:true,message:`aligned by learned_bc (yaw error ${yawError.toFixed(1)}°)`,ticks:i,yawError,policy:'learned_bc',modelId:check.model.modelId||null}}
  ...
  async learnedRetreat(context){
    const s=this.state(context),check=this.validateModel('retreat');if(!check.ok)return{ok:false,reason:check.reason};const referenceYaw=s.robot.yaw,target=this.service(context,'target.retreat',s.robot,70);
    for(let i=0;i<420;i++){
      const r=s.robot,d=context.domainServices?.has?.('world.distance')?context.domainServices.call('world.distance',r,target):Math.hypot(target.x-r.x,target.y-r.y);if(d<=10){await this.act(context,{type:'stop'});if(context.domainServices?.has?.('agent.markRetreated'))context.domainServices.call('agent.markRetreated',true);else{s.agent.memory.retreated=true;this.emit(context)}return{ok:true,message:'retreated (learned_bc)',ticks:i,policy:'learned_bc',modelId:check.model.modelId||null}}
  ...
  async execute(skillId,args={},context={}){
    ...
    if(skillId==='navigate_to_pallet'){const p=s.pallets[args.palletId];if(!p)return{ok:false,reason:'pallet_not_found'};const path=this.service(context,'path.palletApproach',p),m=await this.learnedFollowPath(skillId,path,context,{tolerance:12,maxTicks:900});return m.ok?{ok:true,message:`approached ${p.label} (learned_bc)`,ticks:m.ticks,policy:'learned_bc',modelId:check.model.modelId||null}:{ok:false,reason:m.reason}}
    if(skillId==='navigate_to'){const l=s.locations[args.locationId];if(!l)return{ok:false,reason:'location_not_found'};const target=this.service(context,'target.locationApproach',l),path=this.service(context,'path.to',target),m=await this.learnedFollowPath(skillId,path,context,{tolerance:12,maxTicks:900});return m.ok?{ok:true,message:`followed path to ${l.label} (learned_bc)`,ticks:m.ticks,policy:'learned_bc',modelId:check.model.modelId||null}:{ok:false,reason:m.reason}}
