...
export class MotionBehaviorCloningRuntimeAdapter extends SkillRuntimePolicyAdapter{
  ...
  lengthScale(context){return Math.max(1e-6,(Number(this.control(context)?.wheelbase)||REFERENCE_WHEELBASE)/REFERENCE_WHEELBASE)}
  scaledLength(context,referenceValue){return Number(referenceValue)*this.lengthScale(context)}
  ...
  async learnedDriveTo(skillId,target,context,{targetYaw=null,referenceYaw=null,toleranceRef=12,maxTicks=900}={}){
    const check=this.validateModel(skillId);if(!check.ok)return{ok:false,reason:check.reason};const s=this.state(context),tolerance=this.scaledLength(context,toleranceRef);
  ...
  async learnedFollowPath(skillId,waypoints,context,{toleranceRef=12,maxTicks=900}={}){
    ...
    for(let i=0;i<waypoints.length;i++){s.path.index=i;s.path.lookaheadTarget={...waypoints[i]};this.emit(context);const m=await this.learnedDriveTo(skillId,waypoints[i],context,{toleranceRef,maxTicks});ticks+=m.ticks||0;if(!m.ok){s.path.active=false;s.path.lookaheadTarget=null;this.emit(context);return m}}
  ...
  async execute(skillId,args={},context={}){
    ...
    if(skillId==='navigate_to_pallet'){const p=s.pallets[args.palletId];if(!p)return{ok:false,reason:'pallet_not_found'};const path=await this.serviceAsync(context,'path.palletApproach',p),m=await this.learnedFollowPath(skillId,path,context,{toleranceRef:12,maxTicks:900});return m.ok?{ok:true,message:`approached ${p.label} (learned_bc)`,ticks:m.ticks,policy:'learned_bc',modelId:check.model.modelId||null}:{ok:false,reason:m.reason}}
    if(skillId==='navigate_to'){const l=s.locations[args.locationId];if(!l)return{ok:false,reason:'location_not_found'};const target=await this.serviceAsync(context,'target.locationApproach',l),path=await this.serviceAsync(context,'path.to',target),m=await this.learnedFollowPath(skillId,path,context,{toleranceRef:12,maxTicks:900});return m.ok?{ok:true,message:`followed path to ${l.label} (learned_bc)`,ticks:m.ticks,policy:'learned_bc',modelId:check.model.modelId||null}:{ok:false,reason:m.reason}}
