...
function defaultProfile(skillId,control={}){
  ...
  if(skillId==='align_to_pallet')return{normalizationFamily:'vehicle_relative.v1',forwardScale:3*wheelbase,lateralScale:3*bodyWidth,speedScale:maxForward,actionSpeedScale:Math.min(maxForward,32/55*maxForward),steeringScale:maxSteering};
  if(skillId==='retreat')return{normalizationFamily:'vehicle_relative.v1',forwardScale:3*wheelbase,lateralScale:3*bodyWidth,speedScale:maxReverse,actionSpeedScale:Math.min(maxReverse,26/55*maxForward),steeringScale:maxSteering};
  return{normalizationFamily:'vehicle_relative.v1',forwardScale:10*wheelbase,lateralScale:8*bodyWidth,speedScale:maxForward,actionSpeedScale:Math.min(maxForward,55/55*maxForward),steeringScale:maxSteering};
}

export class MotionSkillIOAdapter extends SkillIOAdapter{
  constructor(){super({id:'motion_skill_io',label:'Motion Vehicle-relative I/O',version:1,observationSpaceId:MOTION_OBSERVATION_SPACE_ID,actionSpaceId:MOTION_ACTION_SPACE_ID})}
  supports(skillId){return['navigate_to_pallet','align_to_pallet','navigate_to','retreat'].includes(skillId)}
  profile(skillId,context={}){
    const services=context.domainServices||context.services,environment=context.environment,control=context.controlConfig||environment?.getState?.()?.simulation||context.state?.simulation||{};
    if(services?.has?.('motion.ioProfile'))return{...defaultProfile(skillId,control),...services.call('motion.ioProfile',skillId)};
    const direct=environment?.getDomainServices?.()?.['motion.ioProfile'];if(typeof direct==='function')return{...defaultProfile(skillId,control),...direct(skillId)};
    return defaultProfile(skillId,control);
  }
  ...
  encodeObservation(skillId,raw,context={}){
    const p=this.profile(skillId,context),yaw=deg2rad(wrap(Number(raw?.yawError)||0));
    return{forward:clamp((Number(raw?.forward)||0)/p.forwardScale,-2,2),lateral:clamp((Number(raw?.lateral)||0)/p.lateralScale,-2,2),yawSin:Math.sin(yaw),yawCos:Math.cos(yaw),speed:clamp((Number(raw?.speed)||0)/p.speedScale,-1.5,1.5),steering:clamp((Number(raw?.steeringAngle)||0)/p.steeringScale,-1,1)};
  }
  ...
  describe(skillId){return{...super.describe(skillId),normalizationFamily:'vehicle_relative.v1',encodedObservation:['forward','lateral','yawSin','yawCos','speed','steering'],encodedAction:['speed','steering'],unitPolicy:'dimensionless model I/O; environment adapter provides vehicle-relative scale profile'}}
}
