...
const motionRuntimeAdapter=new MotionBehaviorCloningRuntimeAdapter(),motionDatasetAdapter=new MotionDatasetAdapter(),motionTrainingBackend=new MotionBcTrainingBackend(),motionDemoRecorder=new MotionDemonstrationRecorderAdapter(motionDatasetAdapter);

class MotionBehaviorCloningPlugin extends SkillLearningPlugin{
  constructor(){super({id:'motion_bc',label:'Motion Behavior Cloning',version:6})}
  supports(skillId){return MOTION_SKILLS.includes(skillId)}
  getCapabilities(){return{trainable:true,evaluable:true,runtimeLearning:true,demonstrationRecording:true,episodicDataset:true,validationSplit:true,policies:['classic','learned']}}
  getAlgorithms(){return[{id:'behavior_cloning',label:'Behavior Cloning',kind:'imitation'}]}
  getDatasetSchema(skillId){return{type:'episodic_observation_action',skillId,episode:['outcome','quality','note'],observation:['dx','dy','yawError','speed','steeringAngle'],action:['speed','steeringAngle']}}
  ...
  getEvaluationMetrics(skillId){const common=[successMetric(),metric('collisionRate','衝突率','percent','',{better:'lower'}),metric('avgControlTicks','平均制御Step','integer','',{better:'lower'}),metric('avgFinalError','最終位置誤差','number','px',{better:'lower'})];if(skillId==='align_to_pallet')common.push(metric('avgYawError','姿勢誤差','number','°',{better:'lower'}));if(skillId==='navigate_to_pallet'||skillId==='navigate_to')common.push(metric('avgPathLength','平均走行距離','number','px',{better:'lower'}));return common}
  getVisualizations(){return[{id:'training_loss',type:'loss_curve',title:'Train / Validation Loss',source:'model.lossHistory'},{id:'dataset_distribution',type:'dataset_distribution',title:'学習データ分布',source:'dataset.featureSummary'},{id:'policy_comparison',type:'policy_comparison',title:'Classic vs Learned',source:'evaluationHistory',metric:'successRate',format:'percent',better:'higher'}]}
  getRuntimePolicyAdapter(){return motionRuntimeAdapter}
  getEvaluationScenarioAdapter(){return motionScenarioAdapter}
  getNote(skillId){return`${skillId} 用の連続制御学習Plugin。Dataset Episode / Demo labels / Train-Validation split / Training Backend / Runtime / Evaluation Scenario / VisualizationをPlugin側で定義します。`}
  async train(skillId,{datasetSource='synthetic_expert',samples=2500,seed=42,epochs=null,timeoutSec=60,validationRatio=.2,demoFilter='all',onProgress=null,signal=null}={}){
    ...
    const datasetRequest=await motionDatasetAdapter.buildTrainingDataset(skillId,{source:datasetSource,samples,seed,validationRatio,demoFilter});onProgress?.({phase:'backend',label:'Web Workerを起動中',progress:0});
    ...
    const identified=await identifySkillModel(skillId,{...trained.model,pluginId:this.id,pluginVersion:this.version,trainingBackendId:trained.backend?.id||motionTrainingBackend.id,trainingBackendVersion:trained.backend?.version||motionTrainingBackend.version,datasetSource,demoFilter,validationRatio,split:trained.dataset.split||datasetRequest.split});
    const model=saveSkillModel(skillId,identified),episodeSummary=motionDatasetAdapter.episodeSummary(skillId);
    const dataset=saveDatasetMeta(skillId,{kind:datasetSource,samples:trained.dataset.samples,trainSamples:trained.dataset.trainSamples,validationSamples:trained.dataset.validationSamples,seed:datasetRequest.seed,generatedAt:new Date().toISOString(),pluginId:this.id,datasetAdapterId:motionDatasetAdapter.id,datasetAdapterVersion:motionDatasetAdapter.version,algorithm:'behavior_cloning',demoFilter,validationRatio,split:trained.dataset.split||datasetRequest.split,episodeSummary,featureSummary:trained.dataset.featureSummary,preview:trained.dataset.preview});
    onProgress?.({phase:'done',label:'学習完了',progress:1});return{model,dataset,pluginId:this.id,trainingBackend:trained.backend};
  }
}

const perceptionPlugin=new DescriptorOnlyLearningPlugin({id:'perception_future',label:'Perception Learning Adapter',version:6,skills:['detect_pallet'],descriptor:{capabilities:{trainable:false,evaluable:true,runtimeLearning:false,demonstrationRecording:false,policies:['classic']},algorithms:[{id:'detector',label:'Detector / Segmentation / VLM',kind:'perception'}],datasetSchema:{type:'image_annotation',observation:['rgb','depth?'],target:['bbox','mask','keypoints','pose']},trainingParameters:[],evaluationParameters:[{key:'trials',label:'Trials',type:'number',default:20,min:1,max:100,step:1},{key:'seed',label:'Seed',type:'number',default:42,step:1}],evaluationMetrics:[successMetric(),metric('avgControlTicks','処理Step','integer','',{better:'lower'})],evaluationScenarioAdapter:perceptionScenarioAdapter,visualizations:[{id:'detection_examples',type:'capability_note',title:'将来の可視化',text:'検出画像 / PR Curve / Confusion Matrix / Pose Error'}],note:'Camera観測を追加したときに、画像Dataset・Annotation Recorder・Detector/VLM学習・知覚Runtime・知覚Scenarioへ差し替えます。'}});
const manipulationPlugin=new DescriptorOnlyLearningPlugin({id:'manipulation_future',label:'Manipulation Learning Adapter',version:6,skills:['insert_forks','lift','place'],descriptor:{capabilities:{trainable:false,evaluable:true,runtimeLearning:false,demonstrationRecording:false,policies:['classic']},algorithms:[{id:'sequence_policy',label:'BC / ACT / Diffusion Policy / RL',kind:'manipulation'}],datasetSchema:{type:'trajectory',observation:['robot_state','fork_state','camera?','depth?'],action:['fork','speed','steering']},trainingParameters:[],evaluationParameters:[{key:'trials',label:'Trials',type:'number',default:20,min:1,max:100,step:1},{key:'seed',label:'Seed',type:'number',default:42,step:1}],evaluationMetrics:[successMetric(),metric('avgControlTicks','平均制御Step','integer','',{better:'lower'})],evaluationScenarioAdapter:manipulationScenarioAdapter,visualizations:[{id:'manipulation_future',type:'capability_note',title:'将来の可視化',text:'Action系列 / 接触位置 / 3D軌跡 / 成功・失敗リプレイ'}],note:'現在のSimulatorでは操作が瞬時状態遷移です。物理自由度を追加後、Recorder/Runtime/Scenarioを含めてPluginを具体実装へ差し替えます。'}});
