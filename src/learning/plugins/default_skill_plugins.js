...
class MotionBehaviorCloningPlugin extends SkillLearningPlugin{
  ...
  async train(skillId,{samples=2500,seed=42,onProgress=null}={}){
    ...
    if(skillId==='align_to_pallet'){
      const bc=new BehaviorCloningAlign();
      model=bc.train(demos,{onEpoch});
      model=saveSkillModel(skillId,{...model,algorithm:'behavior_cloning',pluginId:this.id});
    }else{
      const bc=new BehaviorCloningSkill(skillId);
      model=bc.train(demos,{onEpoch});
    }
