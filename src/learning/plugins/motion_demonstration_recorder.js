...
export class MotionDemonstrationRecorderAdapter extends SkillDemonstrationRecorderAdapter{
  constructor(datasetAdapter){super({id:'motion_manual_demo_recorder',label:'Motion Manual Demonstration Recorder',version:5});this.datasetAdapter=datasetAdapter;this.session=null}
  ...
  start(skillId,{store,replace=false}={}){
    ...
    const context=targetFor(skillId,store.state);this.session={skillId,startedAt:new Date().toISOString(),samples:[],replace,context,store,startRobot:{x:store.state.robot.x,y:store.state.robot.y,yaw:store.state.robot.yaw}};Object.assign(this.session,context);return this.status();
  }
  record(skillId,action,{store}={}){
    ...
    const runtimeStore=store||this.session.store;if(!runtimeStore?.state)return null;const sample={obs:observationFor(skillId,runtimeStore.state,this.session),action:{speed:Number(action.speed),steeringAngle:Number(action.steeringAngle)}};this.session.samples.push(sample);return{sample,count:this.session.samples.length};
  }
  stop(skillId,{save=true,outcome='unlabeled',quality='unrated',note='',store=null}={}){
    ...
    if(session.replace)this.datasetAdapter.clearManualSamples(skillId);const runtimeState=(store||session.store)?.state,endedAt=new Date().toISOString(),episode=this.datasetAdapter.saveEpisode(skillId,{skillId,startedAt:session.startedAt,endedAt,outcome,quality,note,context:{...session.context,startRobot:session.startRobot,endRobot:runtimeState?.robot?{x:runtimeState.robot.x,y:runtimeState.robot.y,yaw:runtimeState.robot.yaw}:null},samples:session.samples});
    const summary=this.datasetAdapter.summarizeManualSamples(skillId),episodeSummary=this.datasetAdapter.episodeSummary(skillId);saveDatasetMeta(skillId,{kind:'manual_recorded',samples:summary.samples,recordedAt:endedAt,datasetAdapterId:this.datasetAdapter.id,datasetAdapterVersion:this.datasetAdapter.version,demonstrationRecorderAdapterId:this.id,demonstrationRecorderAdapterVersion:this.version,episodes:episodeSummary.episodes,episodeSummary,featureSummary:summary.featureSummary||null,preview:summary.preview||null});return{saved:session.samples.length,total:summary.samples,episode,episodeSummary,summary,active:false,startedAt:session.startedAt,endedAt};
  }
