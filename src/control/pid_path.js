...
export class PIDPathController{
  ...
  command(robot,path,{kp=0.8,ki=0.01,kd=0.18,cteGain=1.2,dt=0.08,maxSteeringAngle=35,maxSpeed=85,minSpeed=20}={}){
    if(!path?.length)return{done:true,speed:0,steeringAngle:0,index:0,crossTrackError:0,target:null};
    let nearest=0,best=Infinity;
    for(let i=0;i<path.length;i++){const d=Math.hypot(path[i].x-robot.x,path[i].y-robot.y);if(d<best){best=d;nearest=i}}
    const goal=path[path.length-1];
    if(Math.hypot(goal.x-robot.x,goal.y-robot.y)<10)return{done:true,speed:0,steeringAngle:0,index:path.length-1,crossTrackError:best,target:goal};
    ...
    const cteCorrection=Math.atan2(cteGain*signedCte,Math.max(18,Math.abs(robot.speed))) * 180/Math.PI;
    ...
    const speedScale=Math.max(0.25,1-Math.abs(steering)/Math.max(maxSteeringAngle,1)*0.7);
    const speed=Math.max(minSpeed,maxSpeed*speedScale);
    return{done:false,speed,steeringAngle:steering,index:nearest,crossTrackError:Math.abs(signedCte),target:p1,error,headingError,signedCte};
  }
}
