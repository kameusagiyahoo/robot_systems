...
async function defaultRuntimeFactory({controller='pure_pursuit',environmentId=null}={}){
  ...
  const store=environment.getStore(),state=environment.getState(),robot=environment.getRobot();
  if(!store||!state||!robot)throw new Error(`environment_runtime_contract_incomplete:${id}`);
  ...
  const policy=new RulePolicy(store,robot,{environment}),executor=new SkillExecutor(environment,policy);
  return{environment,environmentDescriptor:environment.describe(),store,robot,policy,executor};
}
...
export async function evaluateSkill(skillId,{trials=20,seed=42,controller='pure_pursuit',environmentId=null,onProgress=null,...pluginOptions}={}){
  ...
  for(let i=0;i<n;i++){
    ...
    const measured=await adapter.measureTrial(skillId,s,prepared,result,{index:i,options,runtime});runs.push({...measured,environmentId:resolvedEnvironmentId});onProgress?.(i+1,n,measured);
    try{await runtime.environment?.disconnect?.()}catch{}
  ...
  const result={...summary,pluginId:getLearningPluginId(skillId),evaluationAdapterId:adapter.id,evaluationAdapterVersion:adapter.version,environmentId:resolvedEnvironmentId,environmentVersion:environmentDescriptor?.version??null,environmentFidelity:environmentDescriptor?.fidelity||null,stateContract:environmentDescriptor?.stateContract||null,modelId:model?.modelId||null,modelChecksum:model?.checksum||null,modelAlgorithm:model?.algorithm||null};
