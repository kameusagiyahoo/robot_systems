...
export function installRemoteEnvironmentUi(){
  ...
  const capabilityState=el('pre','sheet-note');capabilityState.style.whiteSpace='pre-wrap';capabilityState.textContent=known?.capabilities?`Capabilities: ${JSON.stringify(known.capabilities,null,2)}`:'Capabilities: 接続テスト後に取得';details.appendChild(capabilityState);
  const values=()=>({baseUrl:base.value,endpoint:endpoint.value,timeoutMs:Number(timeout.value)||15000});
  testBtn.onclick=async()=>{let config;try{config=saveRemoteEnvironmentConfig(values())}catch(error){status.textContent=error?.message||String(error);return}const checked=await testConfig(config,status);if(checked.ok){capabilityState.textContent=`Capabilities: ${JSON.stringify(checked.descriptor.capabilities||{},null,2)}`}};
