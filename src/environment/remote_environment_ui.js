import {loadRemoteEnvironmentConfig,saveRemoteEnvironmentConfig,saveRemoteEnvironmentDescriptor,clearRemoteEnvironmentConfig,remoteEnvironmentSecurityHint} from './remote_environment_config.js';
...
export function installRemoteEnvironmentUi(){
  ...
  testBtn.onclick=async()=>{let config;try{config=saveRemoteEnvironmentConfig(values())}catch(error){status.textContent=error?.message||String(error);return}await testConfig(config,status)};
