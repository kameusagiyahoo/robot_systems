...
function renderStatus(s){
  ...
  if(status==='done')guide.textContent='成功しました。次はControllerを変えて同じTaskを試せます。';
  else if(['failed','aborted','invalid'].includes(status))guide.textContent='失敗しました。「診断情報をコピー」を押して、その内容をChatGPTに貼ってください。';
  else if(status==='running')guide.textContent='実行中です。シミュレータとAgent Logが更新されます。';
  else if(status==='recovering')guide.textContent='Skill失敗を検知し、Plannerが回復処理を試しています。';
  else guide.textContent='Taskを入力して Run を押してください。';
}
...
function render(s){
  ...
  renderHistory(s);syncControls(s);renderMetrics();renderStatus(s);
}
...
async function run(){if(running||batchRunning)return;running=true;let guard=0;while(running&&guard++<40){const keep=await step();if(!keep||['done','failed','invalid'].includes(store.state.task.status))break;await new Promise(r=>setTimeout(r,350))}if(guard>=40)log('error','Run stopped by 40-step safety guard');running=false;renderStatus(store.state)}
...
log('planner','Ready: v1.6.2 UI clarity + diagnostic copy + stability fixes');
