...
function methodFor(skill){
  const state=skillLearningState(skill.id);
  if(state.policy==='learned'&&state.trained)return state.model?.algorithm==='behavior_cloning'?'BC':'Learned';
  if(skill.id==='navigate_to_pallet'||skill.id==='navigate_to')return controllerLabel();
  return skill.defaultPolicy.replace(' / ','/');
}
...
function renderPipeline(){
  ...
  SKILL_LEARNING_REGISTRY.forEach(skill=>{
    ...
    card.className=`pipeline-skill ${state}`;card.dataset.skill=skill.id;card.title=`${skill.order}. ${skill.code} / ${skill.desc}`;
    const action=skill.trainable
      ?`<a class="skill-learn-link" href="./learn.html?skill=${encodeURIComponent(skill.id)}">${learn.trained?'管理':'学習'}</a>`
      :`<a class="skill-learn-link muted" href="./learn.html?skill=${encodeURIComponent(skill.id)}">詳細</a>`;
  ...
  const all=learningSummary(),trainable=all.filter(x=>x.trainable).length,trained=all.filter(x=>x.trained).length,active=all.filter(x=>x.policy==='learned'&&x.trained).length;
  if($('#pipelineLearnedSummary'))$('#pipelineLearnedSummary').textContent=`学習済 ${trained}/${trainable}・使用中 ${active}`;
  window.dispatchEvent(new CustomEvent('pipeline:model',{detail:{summary:all}}));
}
function install(){
  renderPipeline();const log=$('#log');if(log)new MutationObserver(renderPipeline).observe(log,{childList:true,subtree:true});
  $('#controllerSelect')?.addEventListener('change',renderPipeline);$('#taskInput')?.addEventListener('input',renderPipeline);$('#resetBtn')?.addEventListener('click',()=>setTimeout(renderPipeline,0));
  window.addEventListener('storage',renderPipeline);window.addEventListener('focus',renderPipeline);
}
