...
function evalLabel(skill){
  const state=skillLearningState(skill.id),controller=$('#controllerSelect')?.value||'pure_pursuit';
  const e=latestEvaluationForPolicy(skill.id,state.policy,controller)||latestEvaluationForPolicy(skill.id,state.policy)||state.evaluation;
  return e?`${Math.round((e.successRate||0)*100)}%`:'未評価';
}
function methodFor(skill){
  const state=skillLearningState(skill.id),score=evalLabel(skill);let method;
